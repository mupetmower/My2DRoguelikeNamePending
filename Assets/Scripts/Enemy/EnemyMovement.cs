...
public abstract class EnemyMovement : MonoBehaviour
{
    ...
    public void ChaseTarget(Transform target)
    {
        ...
        } else
        {
            ...
            if (xDir != 0 && target.position.y > transform.position.y)
            {
                newX = 0;
                newY = 1;
                endPos = startPos + new Vector2(newX, newY);
                StartCoroutine(SmoothMovement(endPos));
            }
            else if (xDir != 0 & target.position.y < transform.position.y)
            {
                newX = 0;
                newY = -1;
                endPos = startPos + new Vector2(newX, newY);
            ...
            else if (yDir != 0 && target.position.x > transform.position.x)
            {
                newX = 1;
                newY = 0;
                endPos = startPos + new Vector2(newX, newY);
